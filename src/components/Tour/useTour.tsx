...
export function useTour(tours: TourItem[]): TourType {
    ...
    useEffect(() => {
        ...
        top: 0;
        left: 0;
        transform: translateX(${width / 2 - TOUR_WIDTH / 2}px) translateY(${
            height / 2 - TOUR_HEIGHT
        }px);
        transition: transform .2s ${theme.transitionEasing};
      `
            ...
            if (step > -1 && step < tours.length) {
                ReactDOM.render(
                    <TourCard
                        info={tours[step]}
                        onNext={handleNext}
                        end={step === tours.length - 1}
                    /> ,
                    toursContainerRef.current
                )
            } else {
